            ...
            with conn.cursor() as cur:
                for row_idx, row in enumerate(reader, start=2):
                    try:
                        ...
                        word = row[english_idx].strip() if english_idx < len(row) else ""
                        if not word:
                        ...
                        japanese = row[japanese_idx].strip() if japanese_idx < len(row) and japanese_idx >= 0 else ""
                        
                        # 発音情報を取得
                        pronun = row[pronunciation_idx].strip() if pronunciation_idx < len(row) and pronunciation_idx >= 0 else ""
                        
                        # 日本語と発音を組み合わせる
                        combined_pronunciation = japanese
                        if pronun and pronun != japanese:
                            combined_pronunciation = f"{japanese} [{pronun}]"
                            ...
                            if existing:
                                ...
                                cur.execute("""
                                    ...
                                    SET stage_number = %s,
                                        ...
                                        updated_at = NOW()
                                    WHERE id = %s
                                """, (stage_number, word, combined_pronunciation, audio_url, notes, existing['id']))
                                imported_count += 1
                                continue
                        
                        # データをバッチに追加
                        batch_data.append((grade, question_id, stage_number, word, combined_pronunciation, audio_url, notes))
