...
# データベースからポイント設定を取得
def get_point_event_config(conn, event_type):
    """point_event_typesテーブルからイベントタイプのポイント設定を取得する関数"""
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT name, min_points, max_points, is_automatic, teacher_can_award
        ...
        logging.error(f"Error getting point event config for {event_type}: {e}")
        return None

# すべてのアクティブなポイント設定を取得
def get_all_point_configs(conn):
    """すべてのアクティブなポイント設定を取得する関数"""
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT name, min_points, max_points, is_automatic, teacher_can_award
        ...
        logging.error(f"Error getting all point configs: {e}")
        return []

# ポイント設定に基づくランダムポイント生成
def generate_points_from_config(conn, event_type):
    """データベース設定に基づいてポイントを生成する関数"""
    config = get_point_event_config(conn, event_type)
    if not config:
        logging.warning(f"Point config not found for {event_type}, using default fallback")
        # フォールバック値
        fallback_values = {
            'login': 3,
            'attendance_daily': 10,
            'grade_improvement_small': 20,
            'grade_improvement_medium': 30,
            'grade_improvement_large': 50,
            'attendance_bonus_10days': 50,
            'attendance_bonus_20days': 100
        }
        return fallback_values.get(event_type, 1)
...
# ランダムログインポイント生成（データベース駆動）
def generate_login_points(conn):
    """データベース設定に基づくログインボーナスのポイントを生成する関数"""
    return generate_points_from_config(conn, 'login')
...
def check_and_award_attendance_bonus(conn, user_id, year=None, month=None):
    ...
    try:
        ...
        if attendance_rate == 100:
            # 皆勤ボーナス（100%）- データベース設定を使用
            bonus_points = generate_points_from_config(conn, 'attendance_bonus_20days')
            award_points(
                conn, 
                user_id, 
                bonus_points, 
            ...
            # 90%以上ボーナス - データベース設定を使用  
            bonus_points = generate_points_from_config(conn, 'attendance_bonus_10days')
            award_points(
                conn, 
                user_id, 
                bonus_points, 
        ...
        with conn.cursor() as cur:
            ...
            if birthday.month == today.month and birthday.day == today.day:
                ...
                if cur.fetchone():
                  ...
                  # ボーナス付与 - データベース設定を使用
                birthday_points = generate_points_from_config(conn, 'birthday')
                award_points(
                    conn, 
                    user_id, 
                    birthday_points, 
        ...
        with conn.cursor() as cur:
            ...
            if school_type == 'elementary':
                ...
                if score_difference >= 15:
                    # 15点以上アップ（大）
                    points = generate_points_from_config(conn, 'grade_improvement_large')
                    award_points(
                        conn,
                        user_id,
                        points,
                    ...
                    # 10点以上アップ（中）
                    points = generate_points_from_config(conn, 'grade_improvement_medium')
                    award_points(
                        conn,
                        user_id,
                        points,
                    ...
                    # 5点以上アップ（小）
                    points = generate_points_from_config(conn, 'grade_improvement_small')
                    award_points(
                        conn,
                        user_id,
                        points,
                ...
                # 高校生の場合 - データベース設定を使用
                if score_difference >= 15:
                    points = generate_points_from_config(conn, 'high_school_grade_improvement_large')
                    award_points(
                        conn,
                        user_id,
                        points,
                    ...
                    points = generate_points_from_config(conn, 'high_school_grade_improvement_medium')
                    award_points(
                        conn,
                        user_id,
                        points,
                    ...
                    points = generate_points_from_config(conn, 'high_school_grade_improvement_small')
                    award_points(
                        conn,
                        user_id,
                        points,
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if user['attendance_days']:
                ...
                is_class_day = js_day_of_week in attendance_days or str(js_day_of_week) in user['attendance_days'].split(',')
            
            # 既に付与済みかチェック（授業日に関係なく）
            cur.execute("""
                SELECT id FROM point_history
                WHERE user_id = %s 
            ...
            if cur.fetchone():
              ...
              # ポイント付与（データベース設定を使用）
            attendance_points = generate_points_from_config(conn, 'attendance_daily')
            comment = f"{date}の出席ポイント"
            if is_class_day:
                comment += "（授業日）"
            else:
                comment += "（授業日外）"
